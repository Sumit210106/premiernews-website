...
// 1. Dynamic SEO Metadata Generator for About Us
export async function generateMetadata(): Promise<Metadata> {
  ...
  try {
    ...
    if (!pages || pages.length === 0) return { title: 'About Us' };

    const page = pages[0];
    const seo = page.aioseo_head_json;

    if (!seo) {
      return { title: decodeHtml(page.title?.rendered || 'About Us') };
    }
